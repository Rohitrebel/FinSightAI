...
# 53A. Free Portfolio Deployment Architecture

FinSight is designed to be deployable as a public portfolio/demo application without requiring Docker.
...
Both can be deployed as separate Render web services.
...
- PostgreSQL
- persistent file/object storage for uploaded documents
...
- Twelve Data — primary market data
...
This deployment is for a portfolio/demo application.

### Render

Render currently offers free web-service compute, but free instances are intended for testing, hobby projects and previews rather than production workloads. Free web services cannot use persistent local files; free services also have a monthly Free instance-hours allowance and can spin down when idle. Render's free Postgres is not suitable for our persistent database because free Postgres instances expire after 30 days.

Therefore:

```text
Do not store application data on Render's local filesystem.
Do not use Render Free Postgres as the permanent FinSight database.
```

Use Supabase for persistent relational data and document storage instead.
...
These limits are acceptable for a portfolio/demo dataset.

### Qdrant Cloud

The current Free cluster provides:
...
It is appropriate for our portfolio-sized vector collection.

Free clusters suspend after one week of inactivity and are deleted after four weeks of inactivity if not reactivated.

### Vercel

The current Hobby plan is free and is designed for personal projects.
...
## Production-minded design despite free hosting

The deployment should preserve the same application architecture used locally.
...
The application should not rely on local filesystem state.

## Deployment limitations we accept

For a portfolio demo, the following are acceptable:

- cold starts
- free compute limits
- database/storage quotas
- vector-store inactivity suspension
- external API quotas
- LLM quotas
- occasional manual reactivation after long inactivity

The architecture should remain functional despite these limitations.

## No Docker requirement

Docker is not required for local development or deployment.

The deployment pipeline will use platform-native Node/Python builds.

## Monitoring

Uptime monitoring may be added for the deployed application.

It should not be used as a workaround to force free compute instances to remain permanently awake.
